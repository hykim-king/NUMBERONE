...
<body>
	...
	<div class="container">
				...
				<select name="searchDiv" class="form-select" id="searchDiv">
					...
					<c:forEach var="item" items="${BOARD_SEARCH }">
						<option value="${item.detCode}"
							<c:if test="${item.detCode == search.searchDiv }">selected</c:if>>${ item.detNm}</option>
					</c:forEach>
				</select>
			</div>
			<div class="col-sm-4">
				<input type="search" name="searchWord" class="form-control"
					id="searchWord" value="${search.searchWord }" placeholder="검색어">
			</div>
		</form>
		<!--// 검색 end ------------------------------------------------------------->
		...
		<div class="text-center">
            ...
            Showing <strong><c:out value="${totalCnt }" /></strong> of <strong><c:out
								value="${totalCnt }" /></strong> entries
            </c:if>

					<c:if test="${totalCnt > search.pageSize }">
            Showing <strong><c:out value="${firstCnt }" /></strong> to <strong><c:out
								value="${lastCnt }" /></strong> of <strong><c:out
								value="${totalCnt }" /></strong> entries
            </c:if>
					<ul class="pagination justify-content-center">
						<c:forEach var="item" items="${PAGE_DATA }">
							<li class="page-item <c:if test='${item.active }'>active</c:if>'">
								<a
								href="javascript:pageRetrieve('/ehr/board/doRetrieve.do', ${item.pageNo })"
								class="page-link">${item.pageNo }</a>
							</li>
						</c:forEach>
					</ul>
				</c:if>
